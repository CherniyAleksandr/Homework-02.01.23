class MyArray {
  ...
  const main = () => {
    ...
    console.log("Индекс элемента со значением: ", myArray.find(3));
    console.log("Индекс элемента со значением: ", myArray.find(6));
  
    myArray.removeAtIndex(2);
    console.log("После удаления элемента по индексу: ", myArray.print());
  };
...
const updateFrequencyDict = (char) => {
  const lowerCaseChar = char.toLowerCase(); 
  if (/^[а-яa-z]$/.test(lowerCaseChar)) {
    frequencyDict.set(lowerCaseChar, (frequencyDict.get(lowerCaseChar) || 0) + 1);
  }
};

for (const char of text) {
  updateFrequencyDict(char);
}

frequencyDict.forEach((count, char) => {
  console.log(`Буква "${char}" встречается ${count} раз`);
});